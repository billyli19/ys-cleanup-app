...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (user) {
      ...
      const score = addScore(trashBags);
      const update = {
        trashBags: user.trashBags + trashBags,
      ...
      res
        ...
        .json({ message: "Trash bags have been updated!" });
    } else {
      // Send a JSON error response for invalid credentials
      res
        .status(HTTP_BAD_REQUEST)
        .json({ error: "User not found or something went wrong!" });
    }
  })
);
